...
- IPC: tarpc over Unix socket (`vpn.sock` in app data dir); `create_backend(socket_path, app_handle)` on Android. `start_tunnel` carries the `TunnelParams` and `get_full_info` reports them back (`RunningInfo { params, autonomous }`), so the UI learns a running tunnel's split rules from its owner
