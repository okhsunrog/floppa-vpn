    ...
    <Transition v-else-if="me" name="fade" appear>
      <div>
        ...
        <UCard class="mb-6">
          ...
          <div v-if="hasSubscription">
            ...
            <div class="flex flex-col gap-2">
              <div class="flex items-center gap-2 text-[var(--ui-text-muted)]">
                ...
                <span v-else>{{
                  t('userDashboard.expires', {
                    date: formatDate(me.subscription!.expires_at!),
                    days: daysRemaining,
                  })
                }}</span>
              </div>
              <div class="flex items-center gap-2 text-[var(--ui-text-muted)]">
                ...
                <span>{{
                  t('userDashboard.speed', {
                    speed: me.subscription!.speed_limit_mbps
                      ? `${me.subscription!.speed_limit_mbps} Mbps`
                      : t('common.unlimited'),
                  })
                }}</span>
              </div>
              <div class="flex items-center gap-2 text-[var(--ui-text-muted)]">
                ...
                <span>{{
                  t('userDashboard.maxConfigs', { count: me.subscription!.max_peers })
                }}</span>
              </div>
            </div>
          </div>
          <div
            v-else
            class="flex flex-col items-center gap-2 py-4 text-center text-[var(--ui-text-muted)]"
          >
