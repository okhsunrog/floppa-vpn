export default {
  status: {
    active: 'Активен',
    pending: 'Ожидание',
    removing: 'Удаление',
    removed: 'Удалён',
    connected: 'Подключено',
    connecting: 'Подключение',
    verifyingConnection: 'Проверка соединения',
    disconnected: 'Отключено',
    disconnecting: 'Отключение',
    retrying: 'Переподключение',
    unknown: 'Проверка…',
  },
  traffic: {
    download: 'Скачано',
    upload: 'Загружено',
    unavailable: 'Статистика трафика временно недоступна',
  },
  tray: {
    show: 'Открыть Floppa VPN',
    quit: 'Выход',
  },
  close: {
    title: 'Закрыть окно?',
    descriptionConnected:
      'VPN включён. Floppa VPN может остаться в трее и держать туннель — или отключиться и закрыться.',
    descriptionIdle: 'Floppa VPN может остаться в трее под рукой — или закрыться совсем.',
    remember: 'Запомнить выбор',
    minimize: 'Свернуть в трей',
    quitConnected: 'Отключить и выйти',
    quitIdle: 'Выйти',
  },
  common: {
    cancel: 'Отмена',
    showPassword: 'Показать пароль',
    hidePassword: 'Скрыть пароль',
    save: 'Сохранить',
    delete: 'Удалить',
    remove: 'Удалить',
    close: 'Закрыть',
    copy: 'Копировать',
    download: 'Скачать',
    create: 'Создать',
    error: 'Ошибка',
    success: 'Успешно',
    unlimited: 'Безлимит',
    free: 'Бесплатно',
    yes: 'Да',
    no: 'Нет',
    neverConnected: 'Нет подключений',
    serverUnavailable: 'Сервер недоступен',
    admin: 'Админ',
    user: 'Пользователь',
    copied: 'Скопировано',
    userNumber: 'Пользователь #{id}',
  },
  nav: {
    dashboard: 'Главная',
    myConfigs: 'Мои конфиги',
    admin: 'Админка',
    overview: 'Обзор',
    users: 'Пользователи',
    peers: 'WG пиры',
    vless: 'VLESS',
    installations: 'Устройства',
    plans: 'Тарифы',
    logout: 'Выйти',
    settings: 'Настройки',
    navigation: 'Навигация',
    account: 'Аккаунт',
    info: 'Информация',
    backupNudgeTitle: 'Доступ к Telegram в России ограничен',
    backupNudgeBody: 'Создайте запасной вход, чтобы заходить в Floppa VPN без VPN-подключения.',
    backupNudgeAction: 'Настроить',
    theme: {
      light: 'Светлая',
      dark: 'Тёмная',
      auto: 'Авто',
    },
  },
  login: {
    subtitle: 'Войдите через Telegram',
    configError: 'Не удалось загрузить конфигурацию',
    botNotConfigured: 'Бот не настроен',
    loginFailed: 'Ошибка входа',
    continueInBrowser: 'Продолжить в браузере',
    browserHint: 'Браузер откроет вход через Telegram и автоматически вернёт в приложение.',
    vpnHint: 'Убедитесь, что отключены все активные VPN-соединения перед входом.',
    codeFallbackHint: 'Не вернулись в приложение? Скопируйте код из браузера и вставьте сюда:',
    codePlaceholder: 'Вставьте код входа',
    codeSubmit: 'Войти',
    codeInvalid: 'Неверный или просроченный код. Попробуйте ещё раз.',
    miniAppLoggingIn: 'Вход через Telegram...',
    miniAppFailed: 'Ошибка авторизации Mini App. Попробуйте перезагрузить.',
    deepLinkLoggingIn: 'Завершаем вход...',
    deepLinkFailed: 'Вход не завершился. Попробуйте ещё раз.',
    tabAccount: 'Аккаунт',
    createAccount: 'Создать аккаунт',
    signIn: 'Войти',
    loginPlaceholder: 'Логин',
    passwordPlaceholder: 'Пароль',
    registerHint: 'Сохраните логин и пароль — Telegram можно привязать позже.',
    registerFailed: 'Не удалось создать аккаунт. Проверьте логин и пароль.',
    accountLoginFailed: 'Неверный логин или пароль.',
    loginTaken: 'Этот логин уже занят.',
  },
  apiErrors: {
    forbidden: 'У вас нет прав на это действие.',
    unauthorized: 'Сессия истекла. Войдите заново.',
    tooManyRequests: 'Слишком много запросов. Подождите немного и попробуйте снова.',
    noActiveSubscription: 'Для этого нужна активная подписка.',
    peerLimitReached: 'Достигнут лимит конфигов.',
    vlessNotConfigured: 'VLESS не предлагается этим сервером.',
    amneziawgNotConfigured: 'AmneziaWG не предлагается этим сервером.',
  },
  account: {
    title: 'Аккаунт',
    loginMethods: 'Способы входа',
    linked: 'Привязан',
    linkTelegram: 'Привязать',
    linkWaiting: 'Откройте бота в Telegram, подтвердите и вернитесь сюда…',
    linkError: 'Не удалось начать привязку. Попробуйте снова.',
    alreadyLinked: 'Telegram уже привязан.',
    loginPassword: 'Логин и пароль',
    set: 'Задан',
    notSet: 'Не задан',
    setCredential: 'Завести запасной вход',
    changeCredential: 'Сменить логин или пароль',
    credentialHint:
      'Логин и пароль позволяют войти, даже когда Telegram недоступен. Выберите то, что запомните.',
    loginPlaceholder: 'Логин',
    passwordPlaceholder: 'Пароль',
    save: 'Сохранить',
    credentialError: 'Не удалось сохранить. Проверьте логин и пароль.',
    credentialSaved: 'Сохранено. Теперь можно входить по этому логину и паролю.',
    loginTaken: 'Этот логин уже занят.',
  },
  sessions: {
    title: 'Устройства и сеансы',
    adminTitle: 'Сеансы ({count})',
    hint: 'У каждого устройства, где вы вошли, свой сеанс. Завершайте те, которыми больше не пользуетесь, — или все сразу, тогда выйдете и здесь.',
    empty: 'Нет активных сеансов',
    thisDevice: 'Это устройство',
    unnamedDevice: 'Устройство без имени',
    lastSeen: 'Активность {when}',
    signedIn: 'вход {date}',
    signOut: 'Выйти',
    signOutConfirm: 'Завершить сеанс на этом устройстве? Ему придётся войти заново.',
    signOutCurrentConfirm: 'Завершить сеанс этого устройства? Вы выйдете из аккаунта здесь.',
    signOutEverywhere: 'Выйти везде',
    signOutEverywhereConfirm:
      'Выйти на всех устройствах, включая это? Каждому приложению и браузеру придётся войти заново.',
    signOutFailed: 'Не удалось завершить сеанс',
    signedOut: 'Сеанс завершён',
    signedOutEverywhere: 'Пользователь вышел на всех устройствах',
    adminSignOutConfirm: 'Завершить этот сеанс? Устройству придётся войти заново.',
    adminSignOutEverywhereConfirm:
      'Завершить все сеансы пользователя? Каждому его приложению и браузеру придётся войти заново.',
    kindTelegramWidget: 'Telegram (веб)',
    kindMiniApp: 'Mini App',
    kindDeepLink: 'Приложение',
    kindCredential: 'Логин и пароль',
    kindLegacy: 'Старый вход',
  },
  adminDashboard: {
    title: 'Панель администратора',
    totalUsers: 'Пользователей',
    activePeers: 'Активных пиров',
    subscriptions: 'Подписок',
    totalDownload: 'Всего скачано',
    totalUpload: 'Всего загружено',
    users: 'Пользователи',
    usersDescription: 'Управление пользователями и подписками',
    plans: 'Тарифы',
    plansDescription: 'Настройка тарифных планов',
  },
  adminPeers: {
    title: 'Все пиры',
    searchPlaceholder: 'Поиск по IP, пользователю, устройству...',
    ip: 'IP',
    protocol: 'Протокол',
    region: 'Регион',
    regionUnavailable: 'Этот регион не входит в ваш тариф',
    regionNoVless: 'VLESS недоступен в этом регионе.',
    user: 'Пользователь',
    device: 'Устройство',
    plan: 'Тариф',
    lastSeen: 'Последний раз',
    noPeers: 'Пиры не найдены',
    deletePeer: 'Удалить пир',
    deleteConfirm: 'Удалить пир {ip}? Он будет удалён из WireGuard.',
    peerDeleted: 'Пир удалён',
    deleteFailed: 'Не удалось удалить пир',
    version: 'Версия',
    download: 'Скачано',
    upload: 'Загружено',
  },
  adminVless: {
    title: 'VLESS конфиги',
    searchPlaceholder: 'Поиск по имени...',
    user: 'Пользователь',
    device: 'Устройство',
    version: 'Версия',
    plan: 'Тариф',
    download: 'Скачано',
    upload: 'Загружено',
    noConfigs: 'VLESS конфиги не найдены',
    regenerateTitle: 'Перегенерировать VLESS конфиг',
    regenerateConfirm:
      'Перегенерировать VLESS конфиг для {user}? Текущий конфиг сразу перестанет работать.',
    regenerate: 'Перегенерировать',
    regenerated: 'VLESS конфиг перегенерирован',
    regenerateFailed: 'Не удалось перегенерировать VLESS конфиг',
  },
  adminInstallations: {
    title: 'Устройства',
    searchPlaceholder: 'Поиск по пользователю, устройству, платформе...',
    user: 'Пользователь',
    device: 'Устройство',
    platform: 'Платформа',
    version: 'Версия',
    lastSeen: 'Последний раз',
    noInstallations: 'Устройства не найдены',
    deleteTitle: 'Удалить устройство',
    deleteConfirm: 'Удалить устройство {device}? Оно будет отвязано от пиров.',
    deleted: 'Устройство удалено',
    deleteFailed: 'Не удалось удалить устройство',
  },
  adminUsers: {
    title: 'Пользователи',
    searchPlaceholder: 'Поиск пользователей...',
    id: 'ID',
    username: 'Имя',
    telegramId: 'Telegram ID',
    plan: 'Тариф',
    peers: 'Пиры',
    admin: 'Админ',
    created: 'Создан',
    version: 'Версия',
    noUsers: 'Пользователи не найдены',
    addUser: 'Добавить',
    addUserTitle: 'Предрегистрация пользователя',
    nameLabel: 'Имя (необязательно)',
    namePlaceholder: 'Отображаемое имя до регистрации',
    userCreated: 'Пользователь предрегистрирован',
    createFailed: 'Не удалось создать пользователя',
    userExists: 'Пользователь с таким Telegram ID уже существует',
    idAndTelegram: 'ID: {id} · TG: {tg}',
  },
  adminPlans: {
    title: 'Тарифы',
    addPlan: 'Добавить тариф',
    name: 'Название',
    displayName: 'Отображаемое имя',
    speed: 'Скорость',
    maxPeers: 'Макс. пиров',
    price: 'Цена (⭐)',
    period: 'Период',
    trial: 'Пробный',
    public: 'Публичный',
    actions: 'Действия',
    createPlan: 'Создать тариф',
    editPlan: 'Редактировать тариф',
    internalName: 'Внутреннее имя',
    internalNamePlaceholder: 'напр., basic',
    displayNamePlaceholder: 'напр., Базовый',
    speedLimit: 'Лимит скорости (Мбит/с)',
    priceLabel: 'Цена (⭐)',
    periodDays: 'Период (дни)',
    permanent: 'Постоянный',
    trialDuration: 'Длительность триала',
    notATrial: 'Без пробного периода',
    unitMinutes: 'Минуты',
    unitHours: 'Часы',
    unitDays: 'Дни',
    publicCheckbox: 'Публичный (виден пользователям)',
    planUpdated: 'Тариф обновлён',
    planCreated: 'Тариф создан',
    saveFailed: 'Не удалось сохранить тариф',
    deletePlan: 'Удалить тариф',
    deleteConfirm: 'Удалить тариф "{name}"? Это действие необратимо.',
    planDeleted: 'Тариф удалён',
    deleteFailed: 'Не удалось удалить тариф',
    noPlans: 'Тарифы не настроены',
    days: '{n} дн.',
    hours: '{n} ч',
    minutes: '{n} мин',
  },
  adminUserDetail: {
    backToUsers: 'К списку пользователей',
    id: 'ID',
    telegramId: 'Telegram ID',
    created: 'Создан',
    peers: 'Пиры ({count})',
    noPeers: 'Пиры не настроены',
    vless: 'VLESS',
    vlessNoConfig: 'Нет VLESS конфига',
    vlessActive: 'Активен',
    last: 'Последний: {date}',
    subscription: 'Подписка',
    plan: 'Тариф',
    starts: 'Начало',
    expires: 'Истекает',
    speed: 'Скорость',
    selectPlan: 'Выберите тариф',
    permanent: 'Бессрочная (без истечения)',
    days: 'Дней',
    changeSubscription: 'Изменить подписку',
    createSubscription: 'Создать подписку',
    deleteSubscription: 'Удалить подписку',
    deleteSubscriptionConfirm: 'Удалить активную подписку? Пользователь потеряет доступ к VPN.',
    subscriptionChanged: 'Подписка обновлена',
    recoveryCredential: 'Восстановительный вход',
    setCredential: 'Задать логин и пароль',
    recoveryCredentialHint:
      'Задайте логин и пароль для этого пользователя, чтобы он мог войти без Telegram. Передайте их ему через ваш канал поддержки.',
    credentialSet: 'Логин и пароль заданы',
    credentialFailed: 'Не удалось задать креды',
    changeFailed: 'Не удалось обновить подписку',
    subscriptionDeleted: 'Подписка удалена',
    deleteFailed: 'Не удалось удалить подписку',
    noActiveSubscription: 'Нет активной подписки',
    subscriptionHistory: 'История',
    removePeer: 'Удалить пир',
    removePeerConfirm: 'Удалить пир {ip}?',
    peerRemoved: 'Пир удалён',
    removeFailed: 'Не удалось удалить пир',
    notFound: 'Пользователь не найден',
    device: 'Устройство: {name}',
    sourceTrial: 'Пробный',
    sourceTaster: 'Тестовый',
    sourcePurchase: 'Покупка',
    sourceAdminGrant: 'Админ',
    peerSpeed: 'Скорость: {speed}',
    peerTraffic: 'Трафик: {traffic}',
  },
  userDashboard: {
    title: 'Главная',
    welcome: 'Добро пожаловать,',
    subscription: 'Подписка',
    expires: 'Истекает: {date} ({days} дн.)',
    permanentLabel: 'Бессрочно',
    speed: 'Скорость: {speed}',
    maxConfigs: 'Макс. конфигов: {count}',
    noSubscription: 'Нет активной подписки',
    contactSupport: 'Обратитесь к администратору для активации',
    usage: 'Статистика использования',
    activeConfigs: 'Активных конфигов',
    totalTraffic: 'Общий трафик',
    manageConfigs: 'Управление конфигами',
    appAndPlans: 'Приложения и тарифы',
    adminPanel: 'Панель администратора',
    tasterTitle: 'Пробный доступ — {d}',
    tasterBody: 'Привяжите Telegram, чтобы получить полный 7-дневный пробный период.',
    linkTelegram: 'Привязать Telegram',
  },
  userPeers: {
    title: 'Мои конфиги',
    remaining: '{count} из {max} конфигов доступно',
    createNew: 'Создать конфиг',
    noSubscription: 'Для создания конфигов нужна активная подписка.',
    contactSupport: 'Обратитесь к администратору.',
    noConfigs: 'Конфигов пока нет',
    noConfigsHint: 'Нажмите «Создать конфиг» для начала.',
    lastSeen: 'Последний: {date}',
    showConfig: 'Показать конфиг',
    configTitleFor: 'Конфигурация {protocol}',
    ip: 'IP: {ip}',
    configCreated: 'Конфиг создан',
    configCreatedMessage: 'Новый конфиг успешно создан',
    createFailed: 'Не удалось создать конфиг',
    deleteConfig: 'Удалить конфиг',
    deleteConfirm: 'Вы уверены, что хотите удалить конфиг для {ip}?',
    configDeleted: 'Конфиг удалён',
    configDeletedMessage: 'Конфиг был удалён',
    deleteFailed: 'Не удалось удалить конфиг',
    copiedMessage: 'Конфиг скопирован в буфер обмена',
    device: '{name}',
    createdAt: 'Создан: {date}',
    configSaved: 'Конфиг сохранён в {path}',
    sendToTelegram: 'Отправить в Telegram',
    sentToTelegram: 'Конфиг отправлен в Telegram',
    sendFailed: 'Не удалось отправить конфиг',
    vlessTitle: 'VLESS конфиг',
    vlessShowConfig: 'Показать конфиг',
    vlessRegenerate: 'Перегенерировать',
    vlessRegenerateConfirm: 'Вы уверены? Текущий VLESS конфиг сразу перестанет работать.',
    vlessRegenerated: 'VLESS конфиг перегенерирован',
    vlessCopied: 'VLESS URI скопирован',
    vlessLoadFailed: 'Не удалось загрузить VLESS конфиг',
    vlessRegenerateFailed: 'Не удалось перегенерировать VLESS конфиг',
  },
  settings: {
    title: 'Настройки',
    protocolSelection: 'Протокол',
    autoSelectProtocol: 'Автовыбор протокола',
    autoSelectProtocolHint:
      'Автоматически подбирать самый быстрый рабочий протокол при подключении. Выключите для ручного выбора.',
    localNetwork: 'Локальная сеть',
    allowLocalNetwork: 'Разрешить доступ в обход VPN',
    allowLocalNetworkHint:
      'Оставить приватные и link-local адреса доступными напрямую. Для применения переподключитесь.',
    resetProtocolPreference: 'Сбросить',
    protocolPreferenceReset: 'Предпочтение протокола сброшено',
    protocolPreferenceResetFailed: 'Не удалось сбросить предпочтение протокола',
    configure: 'Настроить',
    protocolSettings: 'Настройки протокола',
    protocolPriority: 'Приоритет',
    protocolPriorityHint:
      'Пробуются сверху вниз; приложение остаётся на первом подключившемся протоколе.',
    currentProtocol: 'Текущий протокол',
    noProtocolYet: 'Пока нет',
    protocolUnavailable: 'недоступен',
    autoSelectHelp:
      'При включённом автовыборе приложение пробует протоколы в указанном порядке и остаётся на первом рабочем. Последний рабочий протокол пробуется первым; «Сбросить» забывает его, и перебор снова начинается сверху. Выключите автовыбор, чтобы выбрать протокол вручную.',
    splitTunneling: 'Раздельное туннелирование',
    lockdownOverridesSplit: 'Android игнорирует эти правила',
    lockdownOverridesSplitHint:
      'Включена «Блокировать соединения без VPN», и система пропускает в сеть только это приложение. Исключённые здесь приложения останутся вообще без связи, а не выйдут напрямую. Выключите эту настройку или добавьте их в список исключений самого Android.',
    splitTunnelingDescription: 'Управление приложениями, использующими VPN',
    modeAll: 'Все приложения',
    modeAllDescription: 'Все приложения через VPN',
    modeInclude: 'Только выбранные',
    modeIncludeDescription: 'Только выбранные приложения через VPN',
    modeExclude: 'Исключить выбранные',
    modeExcludeDescription: 'Все, кроме выбранных, через VPN',
    selectedApps:
      '{count} приложение выбрано | {count} приложения выбрано | {count} приложений выбрано',
    searchApps: 'Поиск приложений...',
    noApps: 'Приложения не найдены',
    showSystemApps: 'Показать системные приложения',
    changesApplyOnReconnect: 'Настройки изменены — переподключитесь для применения',
    applyingChanges: 'Применяем новые правила — туннель перестраивается',
    reconnect: 'Переподключиться',
    batteryOptimization: 'Оптимизация батареи',
    batteryOptimizationDescription:
      'Отключите оптимизацию батареи, чтобы Android не завершал VPN в фоне.',
    batteryDisabled: 'Оптимизация батареи отключена',
    batteryEnabled: 'Оптимизация батареи активна',
    disableBatteryOptimization: 'Отключить',
    batteryPrompt: 'Отключить оптимизацию батареи для стабильной работы VPN в фоне?',
    notifications: 'Уведомления',
    notificationsDescription: 'Включите уведомления, чтобы видеть, когда VPN активен.',
    notificationsOn: 'Уведомления включены',
    notificationsOff: 'Уведомления отключены',
    enableNotifications: 'Включить',
    notificationPrompt: 'Включить уведомления для отображения статуса VPN?',
    windowClose: 'Закрытие окна',
    windowCloseDescription: 'Что делает крестик, пока VPN подключён.',
    closeAsk: 'Спрашивать каждый раз',
    closeTray: 'Оставлять работать в трее',
    closeQuit: 'Отключать VPN и выходить',
    closeQuitServiceHolds: 'Выходить из приложения (VPN продолжит работать)',
    tunnelService: 'Служба туннеля',
    connectOnBoot: 'Подключать при загрузке',
    connectOnBootDetail:
      'Поднимать последний работавший туннель при старте машины, не дожидаясь, пока кто-нибудь войдёт в систему.',
    serviceActive: 'Туннель держит системная служба',
    serviceActiveDetail:
      'Закрытие окна и выход из приложения оставляют VPN подключённым. Отключить можно кнопкой в приложении или командой `floppa disconnect`.',
    serviceAbsent: 'Туннель держит приложение',
    serviceAbsentDetail:
      'VPN отключится вместе с приложением. Чтобы туннель продолжал работать, включите службу: `sudo systemctl enable --now floppa-vpn.socket`.',
    serviceLockedOut: 'Служба туннеля работает, но этому пользователю она недоступна',
    serviceLockedOutDetail:
      'Вы не состоите в группе `floppa`, поэтому приложение держит туннель само и будет спрашивать пароль при каждом подключении. Выполните `sudo usermod -aG floppa $USER` и войдите в систему заново.',
    about: 'О приложении',
    version: 'Версия {version}',
    diagnostics: 'Диагностика',
    diagnosticsDescription:
      'Управляйте уровнем runtime-логов и собирайте диагностические capture для отладки.',
    logProfile: 'Профиль runtime-логов',
    logProfileDescription: 'Управляет логами в adb logcat/stdout без создания файлов.',
    advancedLogFilter: 'Расширенный фильтр',
    advancedLogFilterDescription:
      'Задайте фильтр в формате RUST_LOG. Когда он включен, он заменяет выбранный профиль.',
    customFilterPlaceholder: 'напр. floppa_client_lib=trace,gotatun=debug',
    customFilterEnabled: 'Включить пользовательский фильтр',
    customFilterActive: 'Пользовательский фильтр активен',
    apply: 'Применить',
    logConfigSaveFailed: 'Не удалось сохранить настройки логирования',
    logConfigLoadFailed: 'Не удалось загрузить настройки логирования',
    logCaptureFailed: 'Не удалось переключить диагностический capture',
    logCapture: 'Диагностический capture',
    logCaptureDescription: 'Временно включает verbose-логи и записывает их в файлы capture.',
    logCaptureActive: 'Capture {id} записывается.',
    startLogCapture: 'Начать capture',
    stopLogCapture: 'Остановить capture',
    exportLatestCapture: 'Экспорт последнего capture',
    exportLatestCaptureDescription: 'Экспортирует последний завершенный capture в виде архива.',
    exportLogs: 'Экспорт логов',
    logsExported: 'Логи успешно экспортированы',
    logsExportFailed: 'Не удалось экспортировать логи',
  },
  info: {
    login: 'Войти',
    heroTitle: 'Быстрый VPN, устойчивый к блокировкам',
    heroSubtitle:
      'Floppa VPN держит связь через устойчивый к DPI AmneziaWG по умолчанию, а также WireGuard и VLESS+REALITY — на Android, Windows и Linux.',
    plansTitle: 'Тарифы',
    days: '{n} дней',
    hours: '{n} ч',
    minutes: '{n} мин',
    trialBadge: 'Пробный период {d}',
    devices: '{n} устройств',
    buyViaBot: 'Купить в Telegram-боте',
    downloadsTitle: 'Скачать приложение',
    download_android: 'Android (.apk)',
    download_windows: 'Windows (.exe)',
    download_linux: 'Linux (.AppImage)',
    download_deb: 'Linux (.deb)',
    download_arch: 'Arch (.pkg.tar.zst)',
    channel: 'Telegram-канал',
    bot: 'Telegram-бот',
    github: 'Исходники на GitHub',
    changelogTitle: 'Что нового',
    olderVersions: 'Предыдущие версии',
    version: 'Установленная версия: {version}',
  },
  update: {
    available: 'Доступно обновление: v{version}',
    dismiss: 'Скрыть',
    download: 'Обновить',
    required: 'Требуется обновление',
    requiredDescription: 'Обновите приложение до версии {version} или новее, чтобы продолжить.',
    checkNow: 'Проверить обновления',
  },
  changelog: {
    title: 'Что нового в v{version}',
    whatsNew: 'Что нового',
    notAvailable: 'Список изменений недоступен',
    added: 'Добавлено',
    fixed: 'Исправлено',
    changed: 'Изменено',
    notes: 'Примечания',
  },
  vpn: {
    connect: 'Подключить',
    connecting: 'Подключение...',
    verifyingConnection: 'Проверка...',
    disconnect: 'Отключить',
    disconnecting: 'Отключение...',
    traffic: 'Трафик',
    noSubscription: 'Нет активной подписки',
    peerLimitReached:
      'Достигнут лимит конфигов. Удалите конфиг в «Мои конфиги», чтобы освободить место.',
    peerCreateFailed: 'Сервер отказался создать конфиг: {detail}',
    reconnecting: 'Соединение потеряно. Переподключение ({current}/{max})...',
    retryingIn: 'Следующая попытка через {seconds} с',
    waitingForNetwork: 'Ожидание сети',
    resumesWhenOnline: 'Подключится само, как только на устройстве появится сеть.',
    noNetwork: 'На устройстве нет сети',
    noNetworkHint: 'Туннель поднят и начнёт пропускать трафик, как только появится связь.',

    offlineMode: 'Автономный режим',
    offlineModeHint: 'Сервер недоступен. Используется сохранённая конфигурация.',
    offlineModeNoConfig: 'Сервер недоступен. Для первой настройки VPN нужна связь с сервером.',
    noDeviceIdentity:
      'Не удалось определить это устройство, поэтому ключи VPN настроить нельзя. Перезапустите приложение; если повторяется — проверьте, что приложение может писать в свою папку настроек.',
    serviceNotAnswering:
      'Служба VPN не отвечает. Туннель, возможно, ещё работает — переподключитесь, если трафик прекратится.',
    adoptedTunnel: 'Этот туннель запущен вне приложения (always-on VPN).',
    alwaysOnConfigured:
      'Always-on VPN: система перезапустит его после перезагрузки или обновления.',
    lockdownOn: 'Включена блокировка соединений без VPN',
    lockdownHint:
      'После отключения на устройстве не будет интернета совсем — пока вы не подключитесь снова или не выключите эту настройку.',
    retry: 'Повторить',
    connectionFailed: 'Не удалось подключиться. Попробуйте позже.',
    tryingProtocol: 'Пробую {protocol}… ({current}/{total})',
    connectedVia: 'через {protocol}',
    cancel: 'Отмена',
    lastActivity: 'Последний трафик',
    duration: 'Длительность',
    protocol: 'Протокол',
    region: 'Локация',
    regions: {
      europe: 'Европа',
      moscow: 'Москва',
      singapore: 'Сингапур',
    },
    wireguard: 'WireGuard',
    amneziawg: 'AmneziaWG',
    vless: 'VLESS',
    server: 'Сервер',
    errors: {
      emptyOrder: 'Не выбран протокол для подключения.',
      noUsableConfig: 'Нет конфига для выбранного протокола. Для настройки нужна связь с сервером.',
      actorGone: 'Движок VPN не запущен. Перезапустите приложение.',
      settleTimeout: 'Туннель не стабилизировался вовремя. Попробуйте ещё раз.',
      emptyConfig: 'Полученный с сервера конфиг пуст.',
      unparseableConfig: 'Не удалось прочитать конфиг с сервера: {detail}',
      unexpected: 'Непредвиденная ошибка: {detail}',
      unwindFailed:
        'Не удалось корректно закрыть туннель. Если сеть не работает, перезапустите приложение.',
      attemptFailed: '{protocol}: {reason}',
      attempt: {
        permissionDenied: 'разрешение на VPN не выдано.',
        consentUnavailable:
          'не удалось показать запрос разрешения на VPN. Откройте приложение и попробуйте снова.',
        noConfig: 'нет конфига для {protocol}.',
        platformUnavailable: 'платформа VPN недоступна: {detail}',
        resolveFailed: 'не удалось разрешить адрес сервера {host}: {detail}',
        invalidConfig: 'конфиг некорректен: {detail}',
        platform: 'не удалось настроить {step}: {detail}',
        backend: 'сбой движка туннеля: {detail}',
        verifyFailed: 'туннель поднялся, но трафик через него не прошёл.',
        timedOut: 'время ожидания подключения истекло.',
        peerStartFailed: 'не удалось запустить пир: {detail}',
        cancelled: 'попытка отменена.',
        crashed: 'попытка подключения аварийно завершилась: {detail}',
      },
      backend: {
        permissionDenied: 'система отклонила привилегированную операцию ({detail})',
        invalidConfig: 'из конфига не удалось собрать туннель ({detail})',
        engine: '{detail}',
        notRunning: 'ничего не запущено',
        serviceUnreachable: 'сервис VPN недоступен ({detail})',
        serviceRefused: 'сервис VPN отказал ({detail})',
        unsupported: 'операция не поддерживается на этой платформе',
      },
      step: {
        prepareLink: 'сетевой интерфейс',
        startBackend: 'движок туннеля',
        address: 'адрес туннеля',
        endpointRoute: 'маршрут до сервера',
        routes: 'маршруты',
        dns: 'DNS',
        androidService: 'сервис VPN Android',
      },
    },
  },
}
