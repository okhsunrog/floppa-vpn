...
async fn main() -> Result<()> {
    ...
    tracing::info!("floppa-daemon starting");

    // Load configuration
    let config = Config::from_env()?;
    let secrets = Secrets::from_env()?;
    tracing::info!(interface = %config.wireguard.interface, "Loaded config");

    // Derive public key from private key
    let wg_public_key = secrets.wg_public_key()?;
    tracing::info!(public_key = %wg_public_key, "Derived WireGuard public key");
    ...
    if let Some(ref rate_limit) = config_for_shutdown.wireguard.rate_limit
        && rate_limit.enabled
    {
        tracing::info!("Cleaning up traffic control rules");
        if let Err(e) = tc::cleanup_tc(&config_for_shutdown.wireguard.interface) {
            tracing::error!(error = %e, "Failed to clean up traffic control");
        }
    }

    tracing::info!("floppa-daemon stopped");
    Ok(())
}
