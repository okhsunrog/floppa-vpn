...
//! holds the other end of [`TunnelControl`](crate::vpn::actor::handle::TunnelControl). Every method
...
mod tests {
    use super::*;
    use crate::vpn::actor::handle::TunnelControl;
    use crate::vpn::actor::types::{IntentView, Phase, TunnelParams};
    use crate::vpn::remote::{RemoteActor, TunnelProcess};
    use std::sync::Arc;
    ...
    impl crate::vpn::actor::handle::TunnelControl for FakeActor {
    ...
    fn spawner() -> crate::vpn::actor::Spawn {
        Arc::new(|task| {
            tokio::spawn(task);
        })
    }
    ...
    async fn the_mirror_follows_the_actor_across_a_restart_of_the_process_holding_it() {
        ...
        let path = dir.path().join(crate::vpn::rpc::SOCKET_NAME);
    ...
    async fn every_operation_crosses_and_comes_back_typed() {
        ...
        let process = Arc::new(AlwaysRunning(AtomicUsize::new(0)));
        let remote = RemoteActor::new(dir.path(), process.clone(), &spawner());
        use crate::vpn::actor::handle::TunnelControl as _;
